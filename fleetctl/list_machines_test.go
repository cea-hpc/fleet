...
func TestListMachinesFieldsToStrings(t *testing.T) {
	...
	assertEqual(t, "metadata", "foo=bar,hostname=machineHostname,ping=pong", val)

	val = listMachinesFields["hostname"](ms, false)
	assertEqual(t, "hostname", "machineHostname", val)
}
